/**
 * NOTE: this deliberately does NOT use ESLint's `extends: ['@desavii/config/...']`
 * string form. ESLint 8's shareable-config naming convention silently
 * rewrites scoped package names in `extends` (e.g. `@desavii/config`
 * is treated as shorthand for `@desavii/eslint-config-config`), which
 ...
 * convention already.
 */
const backendConfig = require('@desavii/config/src/eslint-backend.cjs');
